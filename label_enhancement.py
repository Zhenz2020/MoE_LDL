...
if __name__ == '__main__':
    # 示例数据
    points = np.array(pd.read_csv('data/simulation_data.csv').iloc[:, 0:10].values)
    y = np.array(pd.read_csv('data/simulation_data.csv')['true'].values)
    ...
    # 计算 KL 散度
    kl_loss = tf.multiply(result['weight'],(0.5 * (log_var_pred - log_var_true + tf.exp(log_var_true - log_var_pred) + tf.square(
        mean_true - mean_pred) / tf.exp(log_var_pred) - 1.0)))
    draw_mix_gaussion(result['mean'][0], 1 / result['weight'][0] * 4, result['weight'][0])
    # print(result['mean'][0], result['weight'][0])
    out=tf.reduce_mean(kl_loss)